...
from models import (
    db, Task, TaskUpdate, TaskFile, TaskUpdateLink, ProjectMember,
    Task as TaskModel
)
...
# -----------------------------------------------------------------------------
# Cloudinary (ไม่บังคับใช้ใน flow unsigned แต่คงไว้เผื่อใช้ลบไฟล์/ดู config)
# -----------------------------------------------------------------------------
api_key     = get_env("CLOUDINARY_API_KEY")
api_secret  = get_env("CLOUDINARY_API_SECRET")
cloud_name  = get_env("CLOUDINARY_CLOUD_NAME")
upload_preset = get_env("CLOUDINARY_UPLOAD_PRESET")

if cloud_name:
    # คง config ไว้ (ใช้เวลา destroy ตอนลบไฟล์)
    cloudinary.config(
...
# -----------------------------------------------------------------------------
# Task Feed (หน้าแสดง/อัปเดตงาน) — ส่งค่า Cloudinary ให้ template ใช้ใน JS
# -----------------------------------------------------------------------------
...
def task_feed(task_id):
    ...
    return render_template(
        ...
        # >>> ค่าที่ JS ต้องใช้สำหรับ unsigned upload <<<
        cloudinary_cloud_name=get_env("CLOUDINARY_CLOUD_NAME"),
        cloudinary_upload_preset=get_env("CLOUDINARY_UPLOAD_PRESET"),
    )

# -----------------------------------------------------------------------------
# (Option เผื่อหน้าเพจที่ต้องการคอนฟิกแบบ JSON) 
# GET /tasks/<task_id>/upload-config -> {"cloud_name": "...", "upload_preset": "..."}
# -----------------------------------------------------------------------------
@tasks_bp.get('/tasks/<int:task_id>/upload-config')
@login_required
def task_upload_config(task_id):
    # แค่ยืนยันสมาชิกของโปรเจ็กต์ก่อน
    t, _mem = _task_and_membership(task_id)
    return jsonify({
        "cloud_name": get_env("CLOUDINARY_CLOUD_NAME") or "",
        "upload_preset": get_env("CLOUDINARY_UPLOAD_PRESET") or "",
    })

# -----------------------------------------------------------------------------
# Create Update (โพสต์อัปเดต + แนบลิงก์ที่ JS เติมให้)
# -----------------------------------------------------------------------------
...
def create_update(task_id):
    ...
    # อนุญาต owner/ba ปรับ % และ สถานะ
    ...
    db.session.add(upd)
    db.session.flush()  # ให้ได้ upd.id

    # แนบลิงก์ (JS จะเติม secure_url ของ Cloudinary มาทีละบรรทัด)
...
# -----------------------------------------------------------------------------
# Register a Cloudinary file (บันทึกเมตาดาต้าหลังอัปโหลดเสร็จ)
# ถูกเรียกจาก JS หลัง Cloudinary ส่ง URL กลับมาแล้ว
# -----------------------------------------------------------------------------
...
def register_uploaded_file(task_id):
    ...
    # (ปลอดภัยขึ้นนิด) ถ้ามี cloud_name ให้เช็คว่า URL มาจาก Cloudinary เดียวกัน
    cn = get_env("CLOUDINARY_CLOUD_NAME") or ""
    if cn and ("res.cloudinary.com/{}/".format(cn) not in secure_url):
        # ไม่บังคับ reject แต่แจ้งเตือน/ป้องกันลิงก์ outsider
        pass
...
# -----------------------------------------------------------------------------
# Delete File (optionally ลบที่ Cloudinary ด้วยหากเปิด ENV อนุญาต)
# -----------------------------------------------------------------------------
...
def delete_file(task_id, file_id):
    ...
    # ลบไฟล์ที่ Cloudinary ถ้าตั้งค่าอนุญาต (และมี public_id)
    if is_cloudinary_delete_enabled() and f.public_id:
        try:
            # resource_type="raw" เผื่อไฟล์ non-image เช่น pdf/docx/xlsx
            cloudinary.uploader.destroy(
                f.public_id, invalidate=True, resource_type="raw"
            )
        except Exception:
            pass
