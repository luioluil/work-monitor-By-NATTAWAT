...
from utils import (
    current_user, login_required, must_be_project_member,
    is_cloudinary_delete_enabled, get_env
)

tasks_bp = Blueprint('tasks', __name__)

# ---------------- Cloudinary config (optional สำหรับลบไฟล์) ----------------
...
def create_update(task_id):
    ...
    db.session.add(upd)
    db.session.flush()  # ให้ได้ upd.id (ใช้ผูกลิงก์)
...
# ---------------- Delete file (รองรับทั้ง POST และ GET) ----------------
@tasks_bp.route('/tasks/<int:task_id>/files/delete/<int:file_id>', methods=['POST', 'GET'])
@login_required
def delete_file(task_id, file_id):
    ...
    # ตรวจสิทธิ์: owner, BA หรือผู้สร้าง task
    u = current_user()
    ...
    # ลบจาก Cloudinary ถ้าอนุญาตและมี public_id
