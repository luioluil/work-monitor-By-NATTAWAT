...
def create_task(project_id):
    ...
    t = Task(project_id=project_id, title=title, assignee_name=assignee or None,
             created_by_id=u.id, status="todo", progress_percent=0,
...
def task_feed(task_id):
    ...
    updates = TaskUpdate.query.filter_by(task_id=t.id).order_by(TaskUpdate.created_at.desc()).all()
    files = TaskFile.query.filter_by(task_id=t.id).order_by(TaskFile.created_at.desc()).all()
    links_map = {}
    if updates:
    ...
    return render_template('task_feed.html',
                           task=t, updates=updates, files=files, links_map=links_map,
...
def create_update(task_id):
    ...
    db.session.add(upd); db.session.flush()
...
def register_uploaded_file(task_id):
    ...
    task_update_id = request.form.get('task_update_id')
...
def change_status(task_id):
    ...
    if status in ("todo","doing","done","blocked"):
        t.status = status
        t.last_updated = datetime.utcnow()
        db.session.add(TaskUpdate(task_id=t.id, author_id=current_user().id, content=f"เปลี่ยนสถานะเป็น {status}", status=status))
        db.session.commit()
...
def change_progress(task_id):
    ...
    t.progress_percent = pv
    t.last_updated = datetime.utcnow()
    db.session.add(TaskUpdate(task_id=t.id, author_id=current_user().id, content=f"อัปเดตความคืบหน้าเป็น {pv}%", progress_percent=pv))
    db.session.commit()
